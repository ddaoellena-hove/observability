 ...
 *
 * Uses dynamic import('src/components/') with a literal string so Vite
...
export async function loadRegistry(): Promise<Record<string, any>> {
  ...
  try {
    ...
    _loaded = true;
    console.log(`[componentRegistry] Loaded ${Object.keys(registry).length} components from src/components/`);
  } catch (err) {
